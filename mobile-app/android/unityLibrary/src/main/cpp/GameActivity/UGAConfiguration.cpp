...
static void UpdateLocaleCache(
    int (*getter)(char* dst, size_t dst_size, GameActivity* activity, size_t localeIdx),
    ...
    std::string& cache)
{
    ...
    if (getter(buffer, kBufferSize, activity, 0) != 0)
        return;
...
{
    UpdateLocaleCache(GameActivity_getLocaleLanguage, m_AndroidAppInstance->activity, m_CachedLocaleLanguage);
    UpdateLocaleCache(GameActivity_getLocaleCountry, m_AndroidAppInstance->activity, m_CachedLocaleCountry);
}
