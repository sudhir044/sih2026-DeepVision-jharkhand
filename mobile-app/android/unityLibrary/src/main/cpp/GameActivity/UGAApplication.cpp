...
{
    bool gameLoopIsRunnable = m_HasFocus || ShouldRunWithoutFocus();
...
bool Unity::UnityApplication::IgnorePauseEvent() const
{
    ...
    //         Alternatively, another Android activity is launched from the player with the expectation
    //         that the player continues to run without focus.
    // XR: Permission dialog shows up.
    return m_Initialized && (IsInMultiWindowMode() || ShouldRunWithoutFocus());
}
...
void Unity::UnityApplication::OnApplicationCommand(struct android_app *app, int32_t cmd)
{
    ...
    switch (cmd) {
        ...
        case APP_CMD_EDITOR_ACTION:
            ...
            if (unityApp->m_AndroidAppInstance->editorAction == IME_ACTION_DONE) {
                GameActivity_hideSoftInput(unityApp->m_AndroidAppInstance->activity, 0);

                // On API 29 or lower - when in split screen - APP_CMD_SOFTWARE_KB_VIS_CHANGED never comes.
                // Thus Unity doesn't know that keyboard has closed, workaround this issue by manually invoking visibility changed event.
                if (unityApp->IsInMultiWindowMode() && unityApp->GetConfiguration().GetSdkVersion() <= 29) {
                    OnApplicationCommand(app, APP_CMD_SOFTWARE_KB_VIS_CHANGED);
                }
            }
            break;
